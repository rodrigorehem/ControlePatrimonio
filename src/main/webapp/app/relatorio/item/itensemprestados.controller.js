(function() {
    ...
    angular
        ...
        .controller('RelatorioItensEmprestadosController', RelatorioItensEmprestadosController);

    RelatorioItensEmprestadosController.$inject = ['$scope', '$state', 'ItemMovPessoaEmprestimorelatorio','Item', 'ParseLinks', 'AlertService', 'pagingParams', 'paginationConstants','$uibModal'];

    function RelatorioItensEmprestadosController ($scope, $state, ItemMovPessoaEmprestimorelatorio, Item, ParseLinks, AlertService, pagingParams, paginationConstants, $uibModal) {
        var vm = this;
        vm.loadAll = loadAll;
        ...
        vm.exportData = function () {
            ...
            saveAs(blob, "ItensEmprestados.xls");
        };
                
        vm.preencheItem(pagingParams.filtro);
        vm.loadAll();

        function validade(dataDevolucao)
        {
        	...
        	if(dataDevolucao <= dataHoje)
        		{
        			return "color:red";
        		}else if (dataDevolucao > dataHoje)
        			{
        				return "color:green";
        			}       	
        }
        function loadAll (item) {
        	ItemMovPessoaEmprestimorelatorio.query(
            		{
            			filtro: vm.search(),
            ...
            function onSuccess(data, headers) {
                ...
                for(var i = 0; i < data.length; i++)
                	{
                		var itemAux;
                		if(dataTratado[data[i].movimentacao.dataDevolucao])
                			{
                				itemAux = dataTratado[data[i].movimentacao.dataDevolucao];
                			}else { 
                				itemAux = {};
                				itemAux.id = data[i].movimentacao.dataDevolucao;
                        		itemAux.nome = data[i].movimentacao.dataDevolucao
                        		itemAux.itens = new Array();
                        		dataTratado[data[i].movimentacao.dataDevolucao] = itemAux
